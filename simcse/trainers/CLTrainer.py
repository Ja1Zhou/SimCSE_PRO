import collections, math, sys, os, time, warnings, importlib
...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        model = self.model
        for k in self.trainer_args.batcher_function_args:
            batcher_function_args[k] = eval(k)
        get_batcher = getattr(importlib.import_module(f"..{self.trainer_args.batcher_function}", package = f"simcse.trainers.batcher_functions.subpkg"),self.trainer_args.batcher_function)
        batcher_function = get_batcher(**batcher_function_args)
        se = senteval.engine.SE(params, batcher_function, prepare)
        tasks = ['STSBenchmark', 'SICKRelatedness']
        if eval_senteval_transfer or self.model_args.eval_transfer:
